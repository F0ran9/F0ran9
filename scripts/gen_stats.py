...
def main():
    ...
    pub = [r for r in v["repositories"]["nodes"] if not r["isPrivate"]]
    stars = sum(r["stargazerCount"] for r in pub)
    ...
    parts = []
    parts.append(
        f'<svg xmlns="http://www.w3.org/2000/svg" width="920" height="330" viewBox="0 0 920 330" '
        f'font-family="Consolas, \'JetBrains Mono\', monospace">'
    )
    parts.append('<rect width="920" height="330" rx="16" fill="#0D1117" stroke="#30363D"/>')
    parts.append(
    ...
    with open("dist/stats.svg", "w", encoding="utf-8") as f:
        f.write("".join(parts))
    print(f"stats.svg written: total={total} cur={cur} longest={longest} stars={stars} followers={followers}")
